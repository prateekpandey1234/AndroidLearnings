...
That's `offset=20,30,40...90` all coming back empty.

### Check yourself

**1.** Why cache the ranked ID list per *user* (`{user_id}`) rather than one shared cache for everyone?

<details>
<summary>Answer</summary>

Because the ranking *is* personalized — skills, city, language, the listings already applied to. There's no "the results" to share; my page 1 and your page 1 are different lists. A shared cache would serve you my results.

That's also why the cache can't be warmed in advance. It's computed on the first request and thrown away 15 minutes later.
</details>

**2.** Both TTLs are 15 minutes. If you could change only one thing about this caching setup to make the bug rarer, what would it be — and why is it still not a *fix*?

<details>
<summary>Answer</summary>

Shorten the **ID list** TTL. It's the one holding stale references; the content cache is self-healing (miss -> read MongoDB -> repopulate).

But that's a smaller window, not a fix. A listing can expire one second after the list is cached and you'd still serve a dead ID. **Any** TTL > 0 leaves the hole open.

The real fix has two parts: *push* an invalidation when a listing expires instead of waiting out the timer (stops it at the source), and make the API honest when it happens anyway (stops the runaway).
</details>
...
Three pieces of real code. Read them before scrolling. `build_response_data` is **step 3** from Lesson 3.

## Exhibit A — the controller

```python
def build_response_data(self, get_count=False):
    _listing_ids = self.get_listing_id_list()          # the ranked ID list (~4,500)

    _paginator = ArrayPaginator(_listing_ids, self.request)
    _paginated_data = _paginator.fetch()

    content_handler = self.get_content_handler()
    _listing_content = content_handler.get_data(
        _paginated_data["data"],
        language=self._user_language,
    )

    response_data = {
        "data": _listing_content,
...
def fetch(self, **kwargs):
    ...
    if 0 < len(self.result_set["data"]) == self.limit:
        next_url_params = "?offset=" + str(self.offset + self.limit) + "&limit=" + str(self.limit)

    self.result_set["next"] = next_full_url
    self.result_set["count"] = self.count
    return self.result_set
```

## Exhibit C — the content fetcher

```python
def get_data(self, listing_ids, language=..., promoted_listings=None):
    card_data = []
    for _listing_id in listing_ids:
        cache_handler = self._content_cache_cls(_listing_id, language, ...)
        cache_handler.get_configuration()
        content = cache_handler.listing_content

        if not content:
            content = self.find_from_other_source(_listing_id, language)

        if content:
            # ...~30 lines decorating the card...
            card_data.append(content)
...
**How does `next` end up non-null while `data` is `[]`?** Trace one request: `offset=20`, `limit=10`, 4,500 IDs in the list, and all 10 of those IDs belong to listings that expired an hour ago.

<details>
<summary>Hint 1 — Exhibit C first</summary>

Walk the loop with all 10 IDs missing from MongoDB. `content` is falsy every time. What does `card_data` contain when the loop ends? And what happens to those 10 IDs — is there any record they existed?
</details>

<details>
<summary>Hint 2 — order of operations in Exhibit A</summary>

Number the lines in execution order. Which line decides `next`? Which line discovers the content is missing? Which runs **first**?
</details>

<details>
<summary>Hint 3 — the word "data"</summary>

`_paginated_data["data"]` and `response_data["data"]` are four lines apart. Do they hold the same thing?
</details>

## The answer

### 1. `next` is decided before the truth is known

Number Exhibit A's lines by execution:

```python
1.  _listing_ids = self.get_listing_id_list()     # 4,500 IDs
2.  _paginated_data = _paginator.fetch()          # <-- "next" and "count" DECIDED HERE
3.  _listing_content = content_handler.get_data() # <-- content actually fetched HERE
4.  response_data = {...}                         # ships line 2's promise + line 3's reality
```

**Line 2 makes a promise. Line 3 discovers it can't keep it. Nothing goes back and corrects line 2.**

### 2. The paginator is measuring the wrong list

```python
if 0 < len(self.result_set["data"]) == self.limit:
```

That reads "if I produced a full page, there's probably another" — sound logic. But `result_set["data"]` is the **ID slice**: `[84213, 91002, ...]`. Ten integers. It's ten integers whether those listings exist or not.

So the condition asks *"did I slice 10 IDs?"* — never *"did 10 cards render?"* With 4,500 IDs it's true at every offset up to ~4,490. **That's the runaway.** Paging3 is told "more" 449 times in a row.

And note: the paginator **cannot** do better. It was handed a list of integers and a limit. It has no idea content-fetching happens later. It isn't buggy — it's being asked a question it can't answer.
...
if content:
    card_data.append(content)
# <- no else. Nothing. The ID just evaporates.
```

Ten IDs go in, zero cards come out, and **there is no record it happened** — no log, no counter, no exception. `get_data` returns `[]` as if you'd asked for nothing.

That's why it survived in production. Combine it with Lesson 3's alarm watching `count` (trip 1, healthy ~4,500) instead of `len(data)` (trip 2, zero), and the system was **structurally blind** to its own failure.

### 4. The Python detail that let it hide

```python
if not content:
```

From Lesson 4, `get_listing_content_from_mongo` returns `[]` when nothing is found. In Python, `not []` is `True`... and so is `not None`, `not ""`, and `not 0`.

So this one condition silently merges **three different situations**:

| Reality | `content` | `not content` |
|---|---|---|
| Listing exists | `{...}` | `False` |
| Listing not found | `[]` | `True` |
| Lookup never ran | `None` | `True` |

"Not found" and "we don't know" become indistinguishable. In Kotlin you'd be forced to write `content != null` or `content.isEmpty()` — **the compiler would make you say which one you meant.**

### 5. The killer: two meanings of `"data"`

```python
_listing_content = content_handler.get_data(_paginated_data["data"], ...)  # "data" = IDs
...
"data": _listing_content,                                                  # "data" = cards
```

Same key, four lines apart, **two completely different things.** In Exhibit B `"data"` means IDs; in Exhibit A's response it means rendered cards. The paginator counted its `"data"` and the API shipped a promise about the *other* `"data"`.

Kotlin would have made this impossible — `List<Int>` and `List<ListingCard>` are different types. Python dicts are just string keys pointing at anything.

## The real lesson

Read each exhibit alone and **none of them is wrong.**

- **A** orchestrates in a sensible order — paginate, then fetch.
- **B** correctly reports whether it produced a full slice.
- **C** defensively skips content it couldn't load instead of crashing.

Three reasonable files. The bug lives in **the seam between them** — in an assumption nobody wrote down: *"the number of IDs I sliced equals the number of cards that will render."* True 99% of the time. False for 15 minutes after a bulk expiry.

That's what most serious backend bugs look like. Not a wrong line — a wrong assumption at a boundary, invisible in any single file, reachable only when a cache goes stale at the wrong moment.

## One last check

The obvious fix is: *"if the content list is empty, set `next = None`."* Three lines. Stops the runaway immediately.
...
Because it assumes *"this page rendered nothing, therefore nothing more exists"* — and that's false. The ID list is sorted by **relevance**, not by expiry. A dead patch of 10 IDs can easily sit in front of 4,400 perfectly live ones.

Under that patch the user gets an **empty list** and pagination stops — the 4,400 live listings behind it are now unreachable. You'd trade a visible load bug for an invisible recall bug. Strictly worse: nobody notices, and the metric looks great.

The right fix makes `data` and `next` agree by **filling the page**, not by giving up — scan further down the ID list until you actually have 10 renderable cards, and advance `next` by how many IDs you *consumed* rather than by `limit`. Pages stay full, nothing is lost, and it still terminates at the true end of the list.

That's cursor pagination, and it's why `next` should always be treated by clients as an **opaque URL to echo back** — never as something to compute yourself with `offset += limit`.
</details>
...
Five lessons, from "what is a backend" to reading a genuine production bug across five files:

1. A backend is just a program; `urls.py` is a regex lookup table
2. The View is a dumb delegator; `@GET` and `def get()` are two ends of one wire
3. The Controller is a 3-step template; **IDs and content are two separate trips**
4. Three datastores, three questions, and TTLs that let them disagree
5. The bug lives in the seam, and the obvious fix is a trap

The most transferable idea isn't Django-specific: **on a backend, if you didn't add a metric for it, it is invisible.** No screen, no human, no screenshot. That's the real difference from mobile.
